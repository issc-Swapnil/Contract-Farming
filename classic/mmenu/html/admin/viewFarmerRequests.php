...
if(isset($_POST['reject']))
{
  ...
  {
  $query="update contract set status=3 where contractId=".$id[$i];
  $conn->query($query);
  }
}
elseif(isset($_POST['accept']))
{
  ...
  for($i=0;$i<sizeof($id);$i++)
  {
  $query="update contract set status=4,startDate='".$tom."' where contractId=".$id[$i];
  $conn->query($query);
  $query="select reqId,quantity from contract where contractId=".$id[$i];
  ...
  $query1="update contract set endDate='".$end."' where contractId=".$id[$i];
  $conn->query($query1);
  }
}
?>
...
<div class="page">
   <div class="page-header">
            <h1 class="page-title">Requested Contracts</h1>
          </div>
        <div class="page-content container-fluid">
          <div class="panel">
          <div class="panel-body">
          <?php
          $query="select * from contract where status=0";
          $result=$conn->query($query);
          if($row = $result->fetch_assoc())
          {
            ?>
            <form method="post" action="viewFarmerRequests.php">
              ...
              <thead>
                <tr>
                  ...
                  <th>Crop</th>
                  <th>Quantity (Ton)</th>
                  <th>Contracts handled by Farmer</th>
                  <th>Average Rate</th>
                </tr>
              </thead>
              <tbody>
              ...
              do
            {
              ...
              ?>
                <tr>
                  <td><input type="checkbox" name="ids[]" value="<?php echo $row['contractId'];?>"/></td>
          ...
          ?>
            <div >
                  No contracts requested! Send a request to one...
                  <center>
                <div class="col-sm-12 col-md-4 col-xl-2">
                      <div class="example">
                        <ul class="list-unstyled">
                          <li class="mb-20">
                            <button class="btn btn-block btn-primary" id="redirect" onclick="location.href = 'viewFarmerProfiles.php';">Request Contract</button>
                          </li>
                        </ul>
                      </div>
                    </div>
