...
if(isset($_POST['submit']))
{
  ...
  $cQ=$_POST['cquantity'];
  $sQ=$_POST['squantity'];
  $query="insert into crop_supply values(".$crop.",".$supply.",".$cQ.",".$sQ.")";
  if($conn->query($query))
    $error=1;
  else
    $error=2;
...
<html class="no-js css-menubar" lang="en">
  <head>
    ...
    <title>Recommended Supply for Crop</title>
  ...
  <body class="animsition site-navbar-small ">
    ...
    <div class="page">
      <div class="page-header">
        <h1 class="page-title">Add Recommended Supply for Crop</h1>
      </div>
      ...
      <div class="page-content  container-fluid">
        ...
        <!-- Panel Form Elements -->
        <div class="panel">
        <form method="post" action="addCropSupply.php">
          <div class="panel-body">
          <?php
              if ($error==1) {
              ?>
              <div class="d-block small" style="color: green">Recommended Supply for Crop Added</div>
              <?php
              setcookie("error","",time()-1);
              }
              elseif ($error==2)
              { ?>
              <div class="d-block small" style="color: red">Error while adding recommended supply for crop</div>
              <?php
              }
              ?>
              <div class="row">
                <div class="col-md-6 col-lg-6">
                  ...
                  <div class="col-md-6 col-lg-6">
                  <div class="form-group form-material floating" data-plugin="formMaterial">
                    <input type="number" class="form-control" name="cquantity" min="1" value="1" required/>
                    <label class="floating-label">Crop Quantity(Ton)</label>
                  </div>
                  </div>
                  </div>
                  <div class="row">
                  <div class="col-md-6 col-lg-6">
                  <div class="form-group form-material floating" data-plugin="formMaterial">
                    <select class="form-control" name="supply" required>
                    <option></option>
                      <?php
                      $query="select supplyId,supplyName from supply";
                      $result=$conn->query($query);
                      while($row = $result->fetch_assoc())
                      {
                      ?>
                      <option value="<?php echo $row['supplyId']; ?>"><?php echo $row['supplyName']; ?></option>
                      <?php
                      }
                      ?>
                    </select>
                    <label class="floating-label">Supply</label>
                  </div>
                  </div>
                  <div class="col-md-6 col-lg-6">
                  <div class="form-group form-material floating" data-plugin="formMaterial">
                    <input type="number" class="form-control" name="squantity" min="1" value="1" required/>
                    <label class="floating-label">Supply Quantity(Units)</label>
                  </div>
                  </div>
                  </div>
                  <div class="row">
                  <div class="col-sm-12 col-md-4 col-xl-2">
                      <div class="example">
                        <ul class="list-unstyled">
                          <li class="mb-20">
                            <input type="submit" class="btn btn-block btn-primary" name="submit" value="Add" />
                          </li>
                        </ul>
                      </div>
                    </div>
