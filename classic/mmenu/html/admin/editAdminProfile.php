...
{
    $fileWrite = fopen("../common/AboutUs.txt", "w") or die("Unable to open file!");
    fwrite($fileWrite,$_POST['about']);
    fclose($fileWrite);
    $fileWrite = fopen("../common/ContactUs.txt", "w") or die("Unable to open file!");
    $name=$_POST['name'].PHP_EOL;
    $phone=$_POST['phone'].PHP_EOL;
    fwrite($fileWrite,$name);
    fwrite($fileWrite,$phone);
    fwrite($fileWrite,$_POST['address']);
    fclose($fileWrite);
}
?>
...
<div class="page">
    <div class="page-header">
        <h1 class="page-title">Edit Information</h1>
    </div>

    <div class="page-content container-fluid">
      <form method="post" action="editAdminProfile.php" id="myForm">
      
        <div class="panel">
            <div class="panel-body">
            <div class="row">
                <div class="col-md-6 col-lg-6">
                <h4 class="example-title">About</h4>
                <textarea class="form-control" name="about" form="myForm" rows="10" required><?php
                $fileRead = fopen("../common/AboutUs.txt", "r") or die("Unable to open file!");
                while(!feof($fileRead))
                  echo fgets($fileRead);
                fclose($fileRead);
                $fileRead = fopen("../common/ContactUs.txt", "r") or die("Unable to open file!");
              ?></textarea>
                </div>
                <div class="col-md-6 col-lg-6">
                    <div class="row">
                        <div class="col-md-6 col-lg-8">
                            <h4 class="example-title">Company Name</h4>
                            <input type="text" class="form-control" name="name" value="<?php echo fgets($fileRead); ?>" required/>
                        </div>
                        <div class="col-md-6 col-lg-4">
                            <h4 class="example-title">Phone</h4>
                            <input type="text" class="form-control" name="phone" value="<?php echo fgets($fileRead); ?>" required/>
                        </div>
                    </div>
                    <h4 class="example-title">Address</h4>
                    <textarea class="form-control" name="address" form="myForm" rows="5" required><?php
                        while(!feof($fileRead))
                        echo fgets($fileRead);
                        fclose($fileRead);
                        ?></textarea>
